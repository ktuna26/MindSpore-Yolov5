...
class NET(object):
    ...
    def __init_resource(self):
        # if bool(acl.rt.get_device(self.device_id)[1]):
    ...
    def __get_model_info(self,):
        ...
        for i in range(self.output_size):
            ...
            self.class_num = acl.mdl.get_output_dims(self.model_desc, i)[0]['dims'][4]
    ...
    def __callback_func(self, delete_list):
        ...
        for temp in delete_list:
            ...
            for i in range(num):
                ...
                feature_maps.append((acl.util.ptr_to_numpy(output_host, (infer_output_size//4,), 
                                    11).reshape(self.model_output_dims[i])).transpose((0, 3, 1, 2, 4)))
            print('[ACL] moving data takes ', time.time()-t)

            t = time.time()    
            res_tensor = detect(feature_maps, (self.model_input_width, 
                                self.model_input_height), self.class_num)
            print('[Model] detect takes ', time.time()-t)
            
            # Apply NMS
            t = time.time()
            pred = non_max_suppression(res_tensor, conf_thres=0.4, iou_thres=0.5)
            print('[Model] nms takes ', time.time()-t)
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None:
                    ...
                    pass
            print('[Model] the rest takes ', time.time()-t)
        ...
        t = time.time()
        self.__destroy_dataset_and_databuf()
        print('[ACL] resource release takes ', time.time()-t)
        print('[MODEL] callback func stage success')

    
    def run(self, np_imgs, img_dims):
        ...
        self.__data_interaction(np_imgs)
        ...
        t = time.time()
        self.__forward()
        print('[ACL] inference takes ', time.time()-t)
